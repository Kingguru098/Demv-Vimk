...
async def sudolist(client, message):
    await message.reply(
                   photo= random.choice(PHOTO),
                   caption = f"🥂sᴜᴅᴏ ᴜsᴇʀs ᴏғ ʙᴏᴛ ᴀʀᴇ : \n🥂ᴏᴡɴᴇʀ: {OWNER_USERNAME}",
...
async def chatonoff(client: Client, message: Message):
    if not message.from_user:
        ...
        [
            [InlineKeyboardButton(text="ᴇɴᴀʙʟᴇ", callback_data=f"addchat | {chat_id}")],
            [InlineKeyboardButton(text="ᴅɪsᴀʙʟᴇ", callback_data=f"rmchat | {chat_id}")],
        ]
    )
        )
...
@app.on_callback_query(
    filters.regex(
        pattern=r"^(addchat|rmchat)$"
    )
)
async def cb_onoff(Client, query: CallbackQuery):
    callback_data = query.data.strip()
    callback_request = callback_data.split(None, 1)[1]
    chat_id = callback_request.split("|")
    ...
    if user not in (await is_admins(chat_id)):
        ...
        if command == "addchat":
        is_vick = vick.find_one({"chat_id": chat_id})
        if not is_vick:           
            await query.message.edit_text(f"**ᴄʜᴀᴛ-ʙᴏᴛ ᴀʟʀᴇᴀᴅʏ ᴇɴᴀʙʟᴇᴅ**")
        if is_vick:
            vick.delete_one({"chat_id": chat_id})
            await query.message.edit_text(f"**ᴄʜᴀᴛ-ʙᴏᴛ ᴇɴᴀʙʟᴇᴅ ʙʏ** {query.from_user.mention}.")
...
async def chatbotofd(client, message):
    ...
    if not is_vick:
        ...
        await message.reply_text(f"**ᴄʜᴀᴛ-ʙᴏᴛ ᴅɪsᴀʙʟᴇᴅ**")
    if is_vick:
        await message.reply_text(f"**ᴄʜᴀᴛ-ʙᴏᴛ ᴀʟʀᴇᴀᴅʏ ᴅɪsᴀʙʟᴇᴅ**")
